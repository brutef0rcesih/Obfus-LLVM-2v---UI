...
function UploadSidebar({ isOpen, onToggle, activeItem = 'upload' }) {
  ...
  return (
    <div className="relative">
      {/* Sidebar */}
      <div className={`bg-white border-b px-6 py-3 border-r border-gray-300 h-full flex flex-col transition-all duration-300 ease-in-out ${isOpen ? 'w-44' : 'w-16'
        }`}>
              ...
              return (
                ...
                >
                  <div className={`${isOpen ? 'p-5' : 'p-3'} rounded-lg mb-3 ${isActive ? 'bg-gray-300' : 'bg-gray-100 hover:bg-gray-200'
                    ...
                    <IconComponent
                      className={`${isOpen ? 'w-8 h-8' : 'w-6 h-6'} ${isActive ? 'text-gray-700' : 'text-gray-500 group-hover:text-gray-600'
                        } transition-all duration-200`}
                    />
                  </div>
                  {isOpen && (
                    <p className={`text-xs text-center font-medium leading-tight max-w-[120px] ${isActive ? 'text-gray-800' : 'text-gray-600 group-hover:text-gray-700'
                      }`}>
                      {item.label}
                    </p>
                  )}
      ...
      <div className={`absolute top-1/2 transform -translate-y-1/2 transition-all duration-300 ease-in-out ${isOpen ? 'left-40 z-10' : 'left-12 z-10'
        }`}>
        <button
          onClick={onToggle}
